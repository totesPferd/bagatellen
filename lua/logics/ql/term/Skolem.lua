...
package.loaded["logics.ql.term.Skolem"] =  Skolem
...
function Skolem:get_base_spec()
   local val =  self:get_base_term()
   if val
   then
      return val:get_base_spec()
   end
end

function Skolem:get_qualifier()
   local val =  self:get_base_term()
   if val
   then
...
function Skolem:get_qualified(qualifier)
   local val =  self:get_base_term()
   if val
   then
      return val:get_qualified(qualifier)
   end
end
...
function Skolem:__diagnose_single_line(indentation)
   indentation:insert(String:string_factory("(logics::ql::term::Skolem "))

...
def get_val(json_data, token):
   retval =  0

   if token in json_data["scores"]:
      retval =  json_data["scores"][token]

   return retval

def doit(json_accu, json_b_data):

   for token in json_b_data["scores"]:
      json_accu["scores"][token] = get_val(json_accu, token) - json_b_data["scores"][token]

   print(json.dumps(json_accu, sort_keys = True, indent = 3))
